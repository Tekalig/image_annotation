"""
Enhanced Part 2: Simple CNN with Custom Dataset Support
Now supports loading custom image datasets from /datasets/part2 folder
"""
...
class SimpleCNN(nn.Module):
    ...
    - Fully Connected: 64*8*8 -> 128 -> num_classes
...
def load_custom_dataset(dataset_path='datasets/part2', batch_size=128):
    """
    Load custom image dataset from the datasets folder.
    
    Returns:
        trainloader, testloader, classes
    """
    print(f"Looking for custom dataset in: {dataset_path}")
    
    train_path = os.path.join(dataset_path, 'train')
    test_path = os.path.join(dataset_path, 'test')
    
    if not os.path.exists(train_path) or not os.path.exists(test_path):
        print(f"Custom dataset not found. Expected structure:")
        print(f"  {dataset_path}/train/class1/, class2/, ...")
        print(f"  {dataset_path}/test/class1/, class2/, ...")
    ...
    try:
        # Load training data
        trainset = CustomImageDataset(train_path, transform=transform_train)
        trainloader = DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=2)
        
        # Load test data
        testset = CustomImageDataset(test_path, transform=transform_test)
...
def load_and_test_model(model_path='best_model.pth', testloader=None, classes=None, batch_size=128):
    """
    ...
    # If no testloader provided, we can't do inference
    if testloader is None:
        print("No test data provided for inference")
        return model, None
    ...
    for i in range(min(10, images.size(0))):
        ...
        print(f"{correct} Image {i+1}: True: {true_label:15s} | Pred: {pred_label:15s} | Confidence: {confidence:.2f}%")
    
    # Evaluate on full test set
    accuracy = evaluate_model(model, testloader, classes, device)
...
def suggest_improvements_for_99_percent():
    ...
    print("   ✓ Fine-tune on your dataset")
...
def main():
    """
    Main execution function for Part 2: Simple CNN with custom dataset support
    """
    print("="*70)
    print("PART 2: SIMPLE CNN (Enhanced with Custom Dataset Support)")
    print("="*70)
    ...
    # Step 1: Try to load custom dataset first, fallback to CIFAR-10
    print("\nStep 1: Loading dataset...")
    trainloader, testloader, classes = load_custom_dataset('datasets/part2', batch_size=batch_size)
    
    if trainloader is None:
        print("\nNo custom dataset found. Using CIFAR-10...")
    ...
    loaded_model, loaded_acc = load_and_test_model('best_model.pth', testloader, classes, batch_size=batch_size)
    ...
    print("  1. Create folders: datasets/part2/train/ and datasets/part2/test/")
    print("  2. Organize images in class subfolders")
